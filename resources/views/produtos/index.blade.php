            ...
            let html = `
                <div style="display: flex; gap: 20px; align-items: stretch;">
                    <div style="flex-shrink: 0; display: flex; align-items: center; justify-content: center; width: 200px;">
                        ${produto.imagem ? `<img src="${produto.imagem}" style="max-width: 180px; max-height: 100%; object-fit: contain;" alt="Imagem Produto">` : '<div style="width: 180px; height: 100%; display: flex; align-items: center; justify-content: center; color: #888;">Sem imagem</div>'}
                    </div>
                    <div style="flex: 1;">
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    let codigosAtuais = Array.from(document.querySelectorAll('#lista-codigos-ean li')).map(el => el.dataset.codigo);
    ...
    function atualizarLista() {
        fetch('/api/eans')
            ...
            .then(codigos => {
                ...
                if (novos.length > 0) {
                    codigosAtuais.push(...novos);

                    if (alertaVazio) alertaVazio.remove();
                    ...
                    novos.forEach(codigo => {
                        ...
                        item.innerHTML = `
                            <span>${codigo}</span>
                            <div>
                                <button class="btn btn-sm btn-secondary me-2" onclick="mostrarDetalhesProduto('${codigo}')">
                                    <i class="fa-solid fa-info"></i> Detalhes
                                </button>
